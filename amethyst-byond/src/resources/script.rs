...
use amethyst_core::{ecs::{Entity, Resources}, shrev::EventChannel, Time};
...
impl ScriptWorldData {
    pub fn new<S: Into<String>>(source: S, res: &mut Resources) -> Self {
...
impl ScriptComponentFactory {
    pub fn new(res: &mut Resources) -> Self {
        Self {
            denses: res.fetch::<ScriptComponentChannel<Dense>>().clone(),
            dirs: res.fetch::<ScriptComponentChannel<Direction>>().clone(),
            icon_states: res.fetch::<ScriptComponentChannel<IconStateName>>().clone(),
            opaques: res.fetch::<ScriptComponentChannel<Opaque>>().clone(),
        }
    }
